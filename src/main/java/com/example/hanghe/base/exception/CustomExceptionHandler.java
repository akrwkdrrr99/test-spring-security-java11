...
public class CustomExceptionHandler {
    ...
    */
    ...
    public ResponseEntity<Object> handleApiRequestException(BaseException baseEx) {
        String errCode = baseEx.getStatusCode().getStatusCode();
        String errMSG = baseEx.getStatusCode().getStatusMsg();
        BaseResponseBody baseResponseBody = new BaseResponseBody();
        baseResponseBody.setStatusCode(errCode);
        baseResponseBody.setStatusMsg(errMSG);

        // log를 확인하여 어떤 에러가 발생하는지 표시함
        System.out.println("ERR Code : " + errCode + " ERR Msg : " + errMSG);

        return new ResponseEntity(baseResponseBody, baseEx.getStatusCode().getHttpStatus());
    }
}
